...
이미지 파일을 AI가 읽을 수 있도록 base64 형식으로 변환해줘야 합니다.
...
filepath = "./Screenshot_xxxxxxxx_xxxxxx.jpg"  # 원하는 이미지의 경로로 바꿔주세요
...
![image_0](/assets/img/2025-10-03-21-35-50.png){: w="800" }
_영수증 이미지 예시_

Information Extract는 JSON Schema를 사용하여 추출할 정보의 구조를 정의합니다. 각 필드는 **key name**, **type**, **description**으로 구성됩니다.

**🛠️ 주요 타입**

| Type | 설명 | 예시 |
|------|------|------|
| `string` | 텍스트 정보 | "써브웨이 ABCD점" |
| `number` | 숫자 (정수, 소수 포함) | 17500, 3.14 |
| `integer` | 정수만 | 9, -5 |
| `boolean` | 참/거짓 | true, false |
| `array` | 리스트나 튜플 형태의 데이터 | ["사과", "배", "포도"] |
| `object` | 중첩된 구조의 데이터 | {"itemName": "아이스 아메리카노", "quantity": 2} |

**🧾 작성 예시**

**1. 단일 항목 추출: 매장 이름**

영수증에서 "써브웨이 ABCD점"이라는 매장 이름을 추출하려면, 아래처럼 설정할 수 있습니다.
```json
"storeName": {
    "type": "string",
...
```
- **Key name** : `storeName` (원하는 필드명)
- **Type** : `string` (텍스트 정보이므로)

> **스키마 작성 팁** : `storeName`(Key name)과 `description`(설명)을 명확하고 구체적으로 작성할수록 추출 정확도가 높아집니다. 모호하거나 지나치게 일반적인 표현은 피하고, 각 필드의 목적과 예상되는 값을 명확하게 설정하세요!
{: .prompt-tip }
\
**2. 여러 항목 추출: 주문한 메뉴 목록**

메뉴마다 이름, 수량, 가격을 각각 추출하려면, `array` 와 `object` 를 조합하여 테이블 구조를 표현할 수 있습니다.
```json
"menuItems": {
    "type": "array",
...
```
- **Type** : `array` 는 여러 값을 추출할 때 사용하며, **`items` 키워드**로 각 항목의 구조를 정의합니다.
- **items의 type** : `object` 는 여러 속성(이름, 수량, 가격)을 묶을 때 사용하며, **`properties` 키워드**로 세부 필드를 정의합니다.

이렇게 `array` 안에 `object` 를 넣으면 표 형태의 데이터(메뉴 목록)를 메뉴별로 분리하여 추출할 수 있습니다!

#### **4/ 전체 스키마 작성**

이제 배운 내용을 바탕으로 주문 정보(주문 번호, 일시, 매장 이름, 금액, 메뉴 목록 등)를 모두 추출하는 스키마를 작성해 볼까요?
```python
...
```
스키마를 다 작성해보았습니다! 이제 추출된 결과를 확인해 볼까요?
...
```terminal
{
    ...
    "paymentMethod": "쿠페이 머니, 쿠팡캐시"
}
```

결과를 확인해 보면, 주문 정보가 올바르게 추출되었습니다!

주문한 메뉴 목록의 스키마를 작성할 때, 이름과 수량, 가격을 추출하도록 했는데요. \
샌드위치의 여러 옵션도 선택했었는데, `menuItems` 의 `description` 에 **옵션은 무시하라고** 했기 때문에 추출되지 않았습니다.

자신이 정의하는 대로, 자신의 사례에 맞게 커스터마이징을 할 수 있다는 점이 정말 좋은 것 같습니다.😁

## **Information Extract의 장점**

**① 간편한 구현ㅤ② 유연한 스키마 정의ㅤ③ 추가 학습 불필요**
...
Information Extract는 다양한 산업 분야에서 문서 처리 자동화에 활용될 수 있습니다.
...
제가 테스트해 본 결과, 쿠팡이츠뿐만 아니라 배달의 민족이나 요기요의 주문 내역도 잘 추출했습니다. 각기 다른 이미지 구조와 상관없이, 스키마를 정의한 대로 올바르게 추출되어서 굉장히 좋았습니다.🫢
...
Upstage의 Information Extract는 문서에서 정보를 추출하는 작업을 획기적으로 간소화할 수 있습니다. 간단한 스키마 정의만으로 다양한 형식의 문서에서 정확하게 정보를 추출할 수 있다는 점이 가장 큰 장점입니다.
